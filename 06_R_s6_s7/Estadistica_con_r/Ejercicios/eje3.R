...
table(estado_informe)
prop.table(table(estado_informe)) #Calcula las frecuencias relativas
...
dbinom(2,size=10,prob=0.2) #Probabilidad que una binomial(10,0.2) tome el valor 2
...
dev.off()           #Para borrar todo
#con todo esto analizo los datos a ver cómo están y podemos confirmar que sigue una distribución normal

###########################################################
p<-c(0.01,0.025,0.05,0.10,0.9,0.95,0.975,0.99) #La cadena p, con áreas
q<-c(-3,-2,-1,0,1,2,3) #La cadena q, con números
...
#EJERCICIO AVES  
###################################
# Hacer ejercicios con el enlace del profesor que deja en bibliografía
##############################################
