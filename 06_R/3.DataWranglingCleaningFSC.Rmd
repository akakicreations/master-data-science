...
tab_USA %>% filter(continent=="Americas") %>% arrange(murder_rate)
...
```{r}
# Estadísticas de tensión diastolica y siastolica por sexto y edad
NHANES %>% group_by(Gender, AgeDecade) %>%summarize(av_dia=mean(BPDiaAve, na.rm=T), av_sys=mean(BPSysAve, na.rm=T))
#Para quitar los NA
NHANES %>% filter(!is.na(AgeDecade)) %>% group_by(Gender, AgeDecade) %>%summarize(av_dia=mean(BPDiaAve, na.rm=T), av_sys=mean(BPSysAve, na.rm=T))
```
...
billboard %>% 
  pivot_longer(
    cols = starts_with("wk"), 
    names_to = "week", 
    names_prefix = "wk", # Quita el wk de las columnas
    names_ptypes =list(week = integer()), #Lo convierte en integer
    values_to = "rank",
    values_drop_na = TRUE
  )
```
...
pivot_longer(
  ...
  names_to = c("diagnosis", "gender", "age"), #Nombra a 3 columnas diferentes, no solo una
  names_pattern = "new_?(.*)_(.)(.*)", #Con expresiones regulares,lo que está en paréntesis es lo que va a las variables
  values_to = "count" #Los valores van a columna count
)
```
...
```{r}
#? co2
co2_tidy <- co2_wide %>% pivot_longer(-year, #¿qué columnas quiero pivotar?, todas menos el year
                                      names_to="month",
                                      values_to = "co2",
...
```{r}
admissions #Es un dataset
```
* Transformalo usando *pivot_longer()*, una fila para cada major.
```{r}
new_ad <- admissions %>% pivot_wider(names_from= gender,
                           values_from= c(admitted, applicants))
new_ad
```

```{r}
new_ad_mut <- new_ad %>% mutate(rate_women (admitted_women/(admitted_women+applicants_women))*100,
                                rate_women (admitted_women/(admitted_women+applicants_women))*100) %>% #Sin terminar!!!!!!!
